		...
		for (unsigned int i = 0; i < a_pScene->mNumMeshes; ++i)
		{
			...
			for(unsigned int i = 0; i < pMesh->mNumVertices; i++)
    		{
    			const aiVector3D* pTexCoord = pMesh->HasTextureCoords(0) ? &(pMesh->mTextureCoords[0][i]) : &Zero3D;
   				AnimMesh.pTexCoords.push_back(glm::vec3(pTexCoord->x,pTexCoord->y,pTexCoord->z));
    		}
			// AnimMesh.pTexCoords = new glm::vec3[AnimMesh.NumVertices];
	        const aiVector3D* pTexCoord = pMesh->mTextureCoords[i];
			...
			// glm::vec3* tex = new glm::vec3[AI_MAX_NUMBER_OF_TEXTURECOORDS];
			// AnimMesh.pTexCoords = new glm::vec3[AI_MAX_NUMBER_OF_TEXTURECOORDS];
